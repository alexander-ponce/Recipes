...
def create_account():
    ...
    # create the hash()
    if not user_model.Users.validate_user(request.form):
        return redirect ("/")
...
def user_account(id):
    ...
    all_posts= recipe_model.Recipes.get_all_recipes()
    return render_template ('show_user.html', listed_user = one_user, all_posts = all_posts)

@app.route('/login', methods=['POST'])
def login_user():
    if not user_model.Users.validate_login(request.form):
        return redirect('/')
    email_data={
        'email':request.form['login_email']
    }
    
    # recipe_model.Recipes.get_all_recipes()

    returning_user= user_model.Users.get_by_email(email_data)
